...
def create_candidates_with_same_product_code(
        transactions_df, 
        articles_df, 
        week_length=3,
        customers=None,
    ...
    """
    For each customer, create candidates with same product code as last week's purchase.
    ...
    articles_df : pd.DataFrame
        Articles data.
    week_length : int, optional
        Number of weeks to look back, by default 3
    customers : list, optional
        List of customers to consider, by default None
    config : dict
    ...
    -------
    pd.DataFrame
        Candidates dataframe, containing customer_id and article_id.
    
    """
    last_week_number = transactions_df["week_number"].max()
    working_transactions_df = transactions_df.query(f"week_number >= {last_week_number - week_length + 1}")
    working_transactions_df = working_transactions_df.merge(
        articles_df[["article_id", "product_code"]], on="article_id"
    )
    
    working_transactions_df = working_transactions_df[["customer_id", "product_code"]]
    article_product_code_df = articles_df[["article_id", "product_code"]]
    working_transactions_df = working_transactions_df.drop_duplicates()
    article_product_code_df = article_product_code_df.drop_duplicates()
    working_transactions_df = working_transactions_df.merge(
        article_product_code_df, on="product_code", how="left"
    )
    if customers is not None:
        working_transactions_df = working_transactions_df[
            working_transactions_df["customer_id"].isin(customers)
        ][["customer_id", "article_id"]]

    articles_num_purchases_df = (
        transactions_df.groupby(["article_id"])
        .agg(
            spc_last_purchase_week = ("week_number", "max"),
            spc_last_purchase_date = ("t_dat", "max"),
            spc_purchase_count = ("price", "count"),
            spc_purchase_mean = ("price", "mean"),
        )
    )
    features = (["article_id"], articles_num_purchases_df)
    return working_transactions_df, features
...
def create_popular_article_cand(
    ...
    popular_articles_cand = pd.DataFrame(
    ...
    popular_articles_cand = popular_articles_cand.groupby("customer_id").head(num_articles)
...
def create_age_bucket_candidates(
    ...
    buckets_df["age_bucket"] = pd.qcut(buckets_df["age"], age_buckets).cat.codes
    ...
    pi_df = pi_df.groupby(selected_buckets).head(articles)
...
def filter_candidates(candidates, transactions_df, **kwargs):
    ...
    if num_articles is None:
        ...
        most_popular_items = most_popular_items.to_list()
